use crate::{
    model::operation::{register_lift, Command, Movement, Passenger},
    AppState,
};
use axum::{
    extract::{Path, State},
...
pub async fn attach_elevator(
    ...
    tokio::spawn(async move { register_lift(id, cmd_tx).await });
...
pub async fn passenger_request(
    ...
    if let Some(mvm) = Movement::get_direction(request.to_floor, request.from_floor) {
